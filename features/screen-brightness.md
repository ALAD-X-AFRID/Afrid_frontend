...
if "AmbientLightSensor" in window:
  ...
  sensor.start()
```
On web, there is no API to read the user's screen brightness setting. The Ambient Light Sensor API measures room lighting (lux) as a proxy, mapped to 0–1. Returns `null` on unsupported browsers.
...
- Web proxy: Ambient light sensor (MEMS light sensor) — `AmbientLightSensor` API
